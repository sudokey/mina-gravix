...
export function formattedAmount(value?: string | number, decimals?: number, options?: FormattedAmountOptions): string {
    const minus = value && value.toString()[0] === '-' ? '-' : ''
    let amount = new BigNumber(value ?? 0).abs()
    ...
    if (decimals !== undefined && decimals >= 0) {
        amount = (amount.decimalPlaces() ?? 0) > 0 ? amount.dp(decimals, roundingMode) : amount.shiftedBy(-decimals)
    }
